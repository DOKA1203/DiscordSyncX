...
class AccountLinkRepository {
    ...
    fun link(mcUuid: UUID, discordUserId: Long): AccountLink = transaction {
        val now = Instant.now()
        val existing = AccountLinkEntity.find {
    ...
    fun findByMinecraftUuid(mcUuid: UUID): List<AccountLink> = transaction {
        AccountLinkEntity.find { AccountLinks.mcUuid eq mcUuid }
            .map { it.toDTO() }
    }

    fun findByDiscordId(discordUserId: Long): List<AccountLink> = transaction {
        AccountLinkEntity.find { AccountLinks.discordUserId eq discordUserId }
            .map { it.toDTO() }
    }

    fun unlink(mcUuid: UUID, discordUserId: Long) = transaction {
